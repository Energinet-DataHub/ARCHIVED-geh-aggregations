...
namespace GreenEnergyHub.Aggregation.Application.Coordinator.Handlers
{
    public class FlexConsumptionHandler : IAggregationHandler
    {
        ...
        {
            var list = new List<FlexConsumption>();
            foreach (var json in result)
            {
                var obj = JsonSerializer.Deserialize<FlexConsumption>(json);
                list.Add(obj);
            }
            ...
            foreach (var energySupplier in list.GroupBy(hc => hc.EnergySupplierMarketParticipantMRID))
            {
                foreach (var gridArea in energySupplier.GroupBy(e => e.MeteringGridAreaDomainMRID))
                {
                    ...
                    {
                        // If we are the owner of the grid loss metering point we shall not receive this message
                        continue;
                    }

                    var amdts = new AggregatedMeteredDataTimeSeries(CoordinatorSettings.FlexConsumptionName)
                    {
