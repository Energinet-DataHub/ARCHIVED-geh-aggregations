...
def period_mutations(spark, target_dataframe: DataFrame, event_df: DataFrame, cols_to_change):
    ...
    count = joined_mps.where("from_date == effective_date").count()

    # if we have a count of 1 than we've matched an existing period. Otherwise it's a new one
    if count == 1:
        for col_to_change in cols_to_change:
            joined_mps = joined_mps.withColumn(col_to_change, when(col("from_date") == col("effective_date"), col(f"updated_{col_to_change}")).otherwise(col(col_to_change)))
        ...
        update_func_to_date = (when((col("from_date") < col("effective_date")) & (col("to_date") > col("effective_date")), col("effective_date"))
                                .otherwise(col("to_date")))

        # update_func_settlement_method = (when((col("from_date") >= col("effective_date") & ), col(f"updated_{col_to_change}")).otherwise(col(col_to_change)))

        joined_mps = joined_mps.withColumn("old_to_date", col("to_date"))

        periods_df = joined_mps.withColumn("to_date", update_func_to_date) # \
                                        # .withColumn(col_to_change, update_func_settlement_method)

        row_to_add = periods_df \
        .filter(col("to_date") == col("effective_date")) \
        .first()
        ...
        dataframe_to_add = dataframe_to_add.withColumn("to_date", col("old_to_date")).withColumn("from_date", col("effective_date"))
